...
// Use test vector that passes locally
...
// Public inputs in BE format
const inputBase64 = "BbUNpP4upFSSo5lZKpo1ddfTUWJfrvE5YcSVkisu2vM=";
const publicInput = Buffer.from(inputBase64, 'base64');
const publicInputs = [[...publicInput], [...publicInput]];
...
// Use PDA-derived vault token account, NOT associated token account
const [vaultTokenAccount] = PublicKey.findProgramAddressSync([Buffer.from('vault-token'), mint.toBuffer()], programId);

console.log('Testing with known-good proof that passes local tests...');
console.log('Proof bytes length:', proofBytes.length);
console.log('Public input:', publicInput.toString('hex'));
