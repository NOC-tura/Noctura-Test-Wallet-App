...
mod target_arch {
    ...
    const HOST_G2_BYTES: usize = consts::ALT_BN128_PAIRING_ELEMENT_LEN - consts::ALT_BN128_POINT_SIZE;

    /// Convert endianness of 64-byte input (two 32-byte chunks)
    /// EIP-196 uses big-endian, ark-bn254 uses little-endian
    ...
    /// Convert endianness of 128-byte input (two 64-byte chunks for G2)
    #[allow(dead_code)]
    ...
    pub fn alt_bn128_addition(input: &[u8]) -> Result<Vec<u8>, AltBn128Error> {
        ...
        input.resize(consts::ALT_BN128_ADDITION_INPUT_LEN, 0);

        let p = deserialize_g1_be(&input[..consts::ALT_BN128_POINT_SIZE])?;
        let q = deserialize_g1_be(&input[consts::ALT_BN128_POINT_SIZE..])?;
        let sum = (G1Projective::from(p) + G1Projective::from(q)).into_affine();
        serialize_g1_be(&sum)
    }
    ...
    pub fn alt_bn128_multiplication(input: &[u8]) -> Result<Vec<u8>, AltBn128Error> {
        ...
        let mut input = input.to_vec();
        input.resize(consts::ALT_BN128_MULTIPLICATION_INPUT_LEN, 0);

        let point = deserialize_g1_be(&input[..consts::ALT_BN128_POINT_SIZE])?;
        // Scalar is also in big-endian per EIP-196
        let scalar_le = convert_endianness_64(&input[consts::ALT_BN128_POINT_SIZE..consts::ALT_BN128_POINT_SIZE + 32]);
        let scalar = BigInteger256::deserialize_uncompressed_unchecked(&scalar_le[..])
            .map_err(|_| AltBn128Error::InvalidInputData)?;
        let product = (G1Projective::from(point).mul_bigint(scalar)).into_affine();
        serialize_g1_be(&product)
    }
    ...
    pub fn alt_bn128_pairing(input: &[u8]) -> Result<Vec<u8>, AltBn128Error> {
        ...
        let mut accumulator = <Bn254 as Pairing>::TargetField::one();
        for chunk in input.chunks(consts::ALT_BN128_PAIRING_ELEMENT_LEN) {
            let g1 = deserialize_g1_be(&chunk[..consts::ALT_BN128_POINT_SIZE])?;
            let g2 = deserialize_g2_be(&chunk[consts::ALT_BN128_POINT_SIZE..])?;
            accumulator *= Bn254::pairing(g1, g2).0;
        }
        let success = accumulator == <Bn254 as Pairing>::TargetField::one();
        Ok(if success {
            let mut out = vec![0u8; consts::ALT_BN128_PAIRING_OUTPUT_LEN];
            if let Some(last) = out.last_mut() {
                *last = 1;
            }
            out
        } else {
            vec![0u8; consts::ALT_BN128_PAIRING_OUTPUT_LEN]
        })
    }

    /// Deserialize G1 point from EIP-196 big-endian format
    fn deserialize_g1_be(bytes: &[u8]) -> Result<G1Affine, AltBn128Error> {
        if bytes.len() != consts::ALT_BN128_POINT_SIZE {
            return Err(AltBn128Error::InvalidInputData);
        }
        ...
        // Convert BE to LE for ark
        let le_bytes = convert_endianness_64(bytes);
    ...
    /// Deserialize G2 point from EIP-196 big-endian format
    /// EIP-196 format: [x.c1, x.c0, y.c1, y.c0] each 32 bytes BE
    /// ark format: [x.c0, x.c1, y.c0, y.c1] each 32 bytes LE
    fn deserialize_g2_be(bytes: &[u8]) -> Result<G2Affine, AltBn128Error> {
        if bytes.len() != HOST_G2_BYTES {
            return Err(AltBn128Error::InvalidInputData);
        }
        ...
        // Convert from EIP-196 format to ark format
        // EIP-196: [x.c1, x.c0, y.c1, y.c0] BE
        // ark:     [x.c0, x.c1, y.c0, y.c1] LE
        let mut ark_bytes = [0u8; HOST_G2_BYTES];
        // x.c0 (ark[0..32]) = reverse(eip[32..64])
        for i in 0..32 {
            ark_bytes[i] = bytes[63 - i];
        }
        // x.c1 (ark[32..64]) = reverse(eip[0..32])
        for i in 0..32 {
            ark_bytes[32 + i] = bytes[31 - i];
        }
        // y.c0 (ark[64..96]) = reverse(eip[96..128])
        for i in 0..32 {
            ark_bytes[64 + i] = bytes[127 - i];
        }
        // y.c1 (ark[96..128]) = reverse(eip[64..96])
        for i in 0..32 {
            ark_bytes[96 + i] = bytes[95 - i];
        }
        
        let mut reader = &ark_bytes[..];
        let point = G2Affine::deserialize_uncompressed(&mut reader)
            .map_err(|_| AltBn128Error::InvalidInputData)?;
        if !point.is_on_curve() {
            return Err(AltBn128Error::GroupError);
        }
    ...
    /// Serialize G1 point to EIP-196 big-endian format
    fn serialize_g1_be(point: &G1Affine) -> Result<Vec<u8>, AltBn128Error> {
        let mut buffer = Vec::with_capacity(consts::ALT_BN128_POINT_SIZE);
        point
            .serialize_uncompressed(&mut buffer)
            .map_err(|_| AltBn128Error::UnexpectedError)?;
        // ark-bn254 sets flag bits in the high byte of the last field element (Y in LE format)
        // Clear only the highest flag bit (0x80) before converting to BE since EIP-196 doesn't use flags
        // The flag is in byte 63 (last byte of Y coordinate in LE format)
        if buffer.len() >= 64 {
            buffer[63] &= 0x7F; // Clear only the highest bit (flag bit)
        }
        // Convert LE to BE
        Ok(convert_endianness_64(&buffer))
    }
}
...
mod target_arch {
    ...
    pub fn alt_bn128_multiplication(input: &[u8]) -> Result<Vec<u8>, AltBn128Error> {
        ...
        match result {
            0 => Ok(result_buffer.to_vec()),
            err => Err(AltBn128Error::from(err)),
        }
    }
