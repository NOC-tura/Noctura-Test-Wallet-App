...
fn extract_workspace_dependencies(
    workspace_toml: &TomlTable,
) -> Result<BTreeMap<String, String>, Error> {
    Ok(workspace_dep_table(workspace_toml)
        .into_iter()
        .flat_map(|t| t.iter())
...
/// Return a reference to `[workspace.dependencies]`
fn workspace_dep_table(cargo_toml: &TomlTable) -> Option<&TomlTable> {
    cargo_toml
        .get("workspace")
...
fn open_cargo_toml(path: &Path) -> Result<TomlTable, Error> {
...
fn extract_crate_names(
    ...
    let dep_tables = dep_tables(cargo_toml);
    let target_tables = target_dep_tables(cargo_toml);
    let dep_pkgs = dep_tables
        .into_iter()
        .chain(target_tables.into_iter())
        .flat_map(|t| t.iter())
        .filter_map(move |(dep_name, dep_value)| {
            let dep_name = dep_name.as_str();
            let pkg_name = dep_value
                .as_table()
                .and_then(|table| table.get("package"))
...
fn extract_package_name(cargo_toml: &TomlTable) -> Option<&str> {
    cargo_toml
        .get("package")?
        .as_table()?
        .get("name")?
...
fn target_dep_tables<'a>(cargo_toml: &'a TomlTable) -> Vec<&'a TomlTable> {
    let mut tables = Vec::new();
    if let Some(targets) = cargo_toml.get("target").and_then(Value::as_table) {
        for value in targets.values() {
            if let Some(table) = value.as_table() {
                tables.extend(dep_tables(table));
            }
        }
    }
    tables
...
fn dep_tables<'a>(table: &'a TomlTable) -> Vec<&'a TomlTable> {
    let mut tables = Vec::new();
    if let Some(deps) = table.get("dependencies").and_then(Value::as_table) {
        tables.push(deps);
    }
    if let Some(deps) = table.get("dev-dependencies").and_then(Value::as_table) {
        tables.push(deps);
    }
    tables
}
...
mod tests {
    ...
    macro_rules! create_test {
        (
            ...
            #[test]
            fn $name() {
                let cargo_toml: TomlTable = toml::from_str($cargo_toml)
                    .expect("Parses `Cargo.toml`");
                let workspace_cargo_toml: TomlTable = toml::from_str($workspace_toml)
