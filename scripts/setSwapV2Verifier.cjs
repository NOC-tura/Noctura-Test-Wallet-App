...
async function main() {
  ...
  // Load swap_v2 verification key
  const vkeyPath = path.join(__dirname, '..', 'zk', 'keys', 'swap_v2.vkey.json');
  ...
  const [swapV2Verifier] = PublicKey.findProgramAddressSync(
    [Buffer.from('swap-v2-verifier')],
    PROGRAM_ID
  );
  
  console.log('Global State:', globalState.toBase58());
  console.log('Swap V2 Verifier:', swapV2Verifier.toBase58());
  ...
  if (vkeyBytes.length > 900) {
    ...
    for (let i = 0; i < vkeyBytes.length; i += CHUNK_SIZE) {
      ...
      const chunkTx = await program.methods
        .appendSwapV2VerifierChunk(Buffer.from(chunk))
        .accounts({
          admin: admin.publicKey,
          globalState,
          swapV2Verifier,
    ...
    const tx = await program.methods
      ...
      .rpc();
    console.log('Set verifier tx:', tx);
  }
  
  console.log('✅ Swap V2 verifier set successfully!');
}
